  ...
  <div className="bg-[#2B0D03] min-h-screen">
    ...
    <section className="relative px-4 pb-4 md:px-6 md:pb-6 lg:px-8 pt-[88px] z-10 xl:px-12">
      <div className="relative border border-white/20 rounded-[2rem] overflow-hidden backdrop-blur-sm bg-white/5 shadow-2xl transition-all duration-700 hover:border-white/30">
        ...
        <div className="absolute top-[60px] left-0 right-0 h-px bg-gradient-to-r from-transparent via-[hsl(33_38%_64%/0.5)] to-transparent" />

        <div className="container-wide px-5 md:px-8 relative z-10 pt-14 pb-16 md:pt-20 md:pb-24">
          <div className="grid lg:grid-cols-2 gap-10 items-center">
            ...
            >
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <Link to="/contact">
                  <Button variant="gold" className="rounded-xl px-8 h-12 text-[15px] shadow-[0_0_32px_rgba(255,215,0,0.2)] hover:shadow-[0_0_44px_rgba(255,215,0,0.3)] transition-all gap-2 w-full sm:w-auto">
            ...
            >
              {doctors.map((doc, i) => (
                ...
                >
                  <div className="w-[160px] sm:w-[200px] md:w-[220px] rounded-2xl overflow-hidden border-2 border-white/10 group-hover:border-accent/50 transition-colors duration-300">
                    <img
                      src={doc.image}
                      alt={`Dr. ${doc.name}`}
                      ...
                      loading="lazy"
                    />
                    <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-[#2B0D03]/90 via-[#2B0D03]/50 to-transparent p-4 pt-12">
          ...
          >
            {stats.map((s) => (
              <div key={s.label} className="bg-white/[0.06] backdrop-blur-sm rounded-xl p-4 text-center border border-white/10">
      ...
      <div className="container-wide grid sm:grid-cols-3 gap-4">
        {[
          ...
          <ScrollReveal key={item.title} delay={i * 0.1}>
            <div className="bg-white/[0.03] backdrop-blur-sm rounded-2xl p-6 border border-white/10 h-full hover:border-accent/30 transition-colors group">
              <div className="w-11 h-11 rounded-xl bg-accent/10 flex items-center justify-center mb-4 group-hover:bg-accent/20 transition-all">
                <item.icon className="w-5 h-5 text-accent" />
              </div>
              <h3 className="font-heading font-bold text-white text-[15px] mb-2">{item.title}</h3>
